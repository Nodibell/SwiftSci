...
### Added
- **Standard Apache Parquet Compliance (`ParquetReader`, `SwiftDataFrame`)**: Complete pure-Swift support for industry-standard Apache Parquet files generated by PyArrow, DuckDB, Pandas, and Hugging Face Hub. Correctly parses uncompressed Thrift `PageHeader` at the start of each page, handles dictionary pages (`DICTIONARY_PAGE`), decompresses Snappy payload slices per page, unpacks `RLE_DICTIONARY` with variable bit-widths, decodes multi-level repetition/definition levels, and aggregates repeated nested list schemas (`list<item>`). Bit-exact verified with 5,427 rows from Hugging Face `go_emotions`.
- **Pure-Swift NumPy NPY & NPZ Reader (`NPYReader`, `NPZReader`, `SwiftDataFrame`)**: Native zero-dependency reader for `.npy` files and `.npz` archive containers. Supports little-endian multidimensional tensors, ZIP64 extended records, and Deflate decompression via macOS `Compression` framework. Added convenience initializers `DataFrame(npy:)` and `DataFrame(npz:)`.
- **Automatic SQLite Table Discovery (`DataFrame(sqlite:)`, `SwiftDatabase`)**: Added `DataFrame(sqlite: URL, table: String? = nil)` in `SwiftDatabase`, auto-discovering user tables from `sqlite_master` and mapping SQL query results directly into typed DataFrame columns.
- **Quantile Regression / Pinball Loss in GBDT (`GBDTLoss`, `GradientBoostedTreesRegressor`, `SwiftML`)**: Added `GBDTLoss` (`.squaredError`, `.absoluteError`, `.quantile(alpha:)`) to `GradientBoostedTreesRegressor`. Calculates asymmetric pinball negative gradients and per-leaf optimal quantile estimates, allowing automated 80% / 95% non-parametric confidence bands.
- **Extended CLI Format Support (`SwiftSciCLI`)**: Enhanced `swiftsci summary` and `swiftsci convert` to inspect and convert `.parquet`, `.npy`, and `.npz` files alongside CSV and Feather.
...
### Fixed & Accelerated
- **Zero-Allocation Buffer Gather (`SwiftDataFrame`)**: Rewrote `vGather` and generic `gathered(at:)` in `TypedColumn` to use direct C-pointer `UnsafeBufferPointer` memory indexing, eliminating intermediate 100k element `compactMap` heap array allocations during row filtering.
- **`FilterOp` Branch Coverage (`SwiftDataFrame`)**: Added full branch coverage tests for all `FilterOp` sugar cases (`.greaterThan`, `.lessThan`, `.equals`, `.notEquals`, etc.) and fallback scalar mask paths.
- **Codecov CI Configuration**: Added `Tests/**` to `.codecov.yml` ignore list to isolate product code coverage metrics.
