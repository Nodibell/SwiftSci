...
struct DenseGroupingTests {
    private func check(_ key: any AnyColumn) throws {
        ...
        #expect(group.sum()[column: "value", as: Double.self]?.values == [0, 3, nil, 7])
        #expect(group.mean()[column: "value", as: Double.self]?.values == [0, 1.5, nil, 3.5])
        ...
        #expect(group.transform(["value": .sum])[column: "value_group_sum", as: Double.self]?.values == [0, 3, 0, 3, 0, nil, nil, 3, 7, 7])
    }

    @Test func sequentialAccumulation() throws {
