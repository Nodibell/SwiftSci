...
> **Softmax Formulation:** `P(w_i) = exp(z_i / T) / Σ_j exp(z_j / T)`
...
| **Top-P (Nucleus)** | Smallest set with `Σ P(w) ≥ p` | Dynamically adjusts pool size based on confidence |
| **Repetition Penalty** | `z_i ← z_i / θ` if `w_i` in history | Penalizes tokens already generated in current context |
