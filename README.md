...
- **Out-of-Core `ChunkedDataFrame` & Memory Mapping (`SwiftDataFrame`):** Streaming execution pipeline (`ChunkedDataFrame`) yielding lazy chunks with zero-copy POSIX `mmap` partitioning via `MemoryMappedReader`.
- **Pure-Swift Apache Parquet Engine (`SwiftDataFrame`):** Native Parquet format reader and writer with pure-Swift Snappy LZ77 decompressor and Thrift compact protocol parser (`ParquetReader`, `ParquetWriter`, `SnappyDecompressor`).
- **SIMD-Accelerated Hash Joins (`SwiftDataFrame`):** SIMD-accelerated typed hash join algorithms for `Double`, `Int64`, and `String` columns across `.inner`, `.left`, `.right`, and `.outer` relational joins.
- **Quantized LLM Execution on Metal GPU (`SwiftLLM`):** 4-bit and 8-bit quantized matrix multiplication (`QuantizedLinear`) supporting `q4_0`, `q4_1`, `q8_0`, `awq4`, and `awq8` formats, with GGUF dequantizers.
- **Token-Level JSON Grammar Constrained Decoding (`SwiftLLM`):** State machine enforcing valid JSON schemas at token level during autoregressive decoding via `JSONGrammarDecoder.maskLogits`.
- **Dynamic Paged KV-Cache Allocator (`SwiftLLM`):** Unified memory block allocator (`PagedKVCache`) managing fixed-size attention pages (`pageSize: 16`) to prevent memory fragmentation during LLM streaming.
- **YOLOv8-Seg Instance Segmentation (`SwiftVision`):** Prototype mask head (`YOLOSegHead`) producing 32 prototype masks (`160x160`) and mask coefficient regression branches with pixel-level `decodeMask` reconstruction.
- **CLIP Vision-Language Multimodal Projector (`SwiftVision`):** Dual text and vision embedding projection layers with L2-normalization, temperature-scaled cosine similarity, and zero-shot classification on Metal GPU.
- **Autonomous ReAct Agent Reasoning Loop (`SwiftAgent`):** Full multi-step `Thought -> Action -> Action Input -> Observation -> Final Answer` execution loop with `DataFrameAgentTool` and extensible `CustomAgentTool`.
- **100% DocC API Coverage:** Maintained 100.00% public API documentation coverage (1,459 symbols) verified by automated CI.

See [CHANGELOG.md](CHANGELOG.md#340---2026-08-25).
...
| **`SwiftML`**            | Linear/Logistic Regression (LAPACK OLS `dgels_`), Decision Trees, Random Forests, GBDTs, **`LinearSVC`**, **`MLPClassifier`** & **`MLPRegressor`**, **Binary Core ML Exporter (`.mlmodel` / `.mlpackage`)** (Linear, Trees, Forests, MLPs, Scalers), binary **ONNX exporter**, **`SwiftMLError`**. |      [📖](https://nodibell.github.io/SwiftSci/documentation/swiftml/)      |
| **`SwiftCluster`**       | In-memory **`VectorStore`** index, Halko (2011) $O(MNk)$ `RandomizedSVD` for fast `PCA` (`svdSolver: .randomized`), divide-and-conquer SVD (`dgesdd_`), DBSCAN, `IsolationForest`, `LocalOutlierFactor`, `KMeans` with parallel `concurrentPerform` centroid assignment. |    [📖](https://nodibell.github.io/SwiftSci/documentation/swiftcluster/)    |
...
| **`SwiftNLP`**           | NLTK-equivalent engine: `AppleWordTokenizer`, `SentenceTokenizer`, `RegexTokenizer`, `PorterStemmer`, `POSTagger`, `AppleLemmaTagger`, `VADERSentimentAnalyzer`, `NGramTokenizer`, `HashingVectorizer`, actor-based **`NaiveBayesClassifier`** & **`ComplementNaiveBayesClassifier`** (`Codable`), **Local Dense Text Embeddings**. |      [📖](https://nodibell.github.io/SwiftSci/documentation/swiftnlp/)      |
...
| **`SwiftVision`**        | Computer vision & neural inference: **Real YOLOv8n Object Detection**, **YOLOv8-Seg Instance Segmentation** (`YOLOSegHead`), **CLIP Multimodal Projector** (`CLIPProjector`), **`ONNXWeightReader`** Protobuf binary weight parser, `YOLOPreprocessor` (640x640 letterbox), **Deep Convolutional U-Net** segmentation (`UNetArchitecture` on MLX), CNN feature extraction. |    [📖](https://nodibell.github.io/SwiftVision/documentation/swiftvision/)    |
...
## 📊 Complete Performance Comparison (SwiftSci 3.4.0 vs Python)

Official comparative benchmark suite results comparing **SwiftSci 3.4.0** (Release Build `-c release`) against Python data science libraries (**NumPy**, **Pandas**, **Scikit-Learn**, **Statsmodels**, **SHAP**, **PyTorch**, **Ultralytics**) on Apple Silicon (M-series / macOS 15 arm64).
